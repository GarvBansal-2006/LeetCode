...
public:
    int minMoves(vector<string>& classroom, int energy) {
        ...
        vector<vector<vector<vector<bool>>>> vis(m,vector<vector<vector<bool>>>(n,vector<vector<bool>>(energy + 1,vector<bool>(totalMask, false)))); 

        queue<tuple<int, int, int, int>> q;

        int initialMask = (1 << litterCnt) - 1; 
